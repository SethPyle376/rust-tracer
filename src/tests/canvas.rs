...
fn basic_ray_tracing_test() {
    ...
    for y in 0..(canvas_dimension - 1) {
        ...
        for x in 0..(canvas_dimension - 1) {
            ...
            let position = Point::new(world_x, world_y, 10.0);

            let mut direction = &position - &Point::new(0.0, 0.0, -5.0);
            direction.normalize();

            let ray = Ray::new(Point::new(0.0, 0.0, -5.0), direction);
